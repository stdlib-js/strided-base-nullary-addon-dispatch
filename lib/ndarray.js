...
function dispatch( addon, fallback ) {
	if ( !isFunction( addon ) ) {
		throw new TypeError( format( '1fR3c', addon ) );
	}
	if ( !isFunction( fallback ) ) {
		throw new TypeError( format( '1fR2H', fallback ) );
	}
	return dispatcher;
	...
	*/
	function dispatcher( N, dtypeX, x, strideX, offsetX ) {
		...
		if ( dtypeX === null ) {
			throw new TypeError( format('1fR1L') );
		}
		if ( !isNonNegativeInteger( offsetX ) ) {
			throw new TypeError( format( '1fRAE', offsetX ) );
		}
